...
test.describe('Log in functionality', () => {
  ...
  test("It doesn't log in with incorrect password", async ({ page }) => {
    ...
    expect(await commonTests.isErrorMessageVisible('Podane dane logowania są błędne.')).toBe(true);
  });
  ...
  test("It doesn't log in with incorrect email", async ({ page }) => {
    ...
    expect(await commonTests.isErrorMessageVisible('Podany użytkownik nie istnieje.')).toBe(true);
  });
  ...
  test("It doesn't log in without email and password", async ({ page }) => {
    ...
    expect(await commonTests.isErrorMessageVisible('Podany adres email jest nieprawidłowy.')).toBe(
      true
    );
  });
  ...
  test('It validates the email address', async ({ page }) => {
    ...
    for (let email of wrongEmails) {
      ...
      expect(
        await commonTests.isErrorMessageVisible('Podany adres email jest nieprawidłowy.')
      ).toBe(true);
